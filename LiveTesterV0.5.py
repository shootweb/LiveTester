...
# Configure logging level for detailed diagnostics
...
class LiveXSSTester:
    ...
    def append_to_file(self, data, filename=PARAMETERS_OUTPUT_FILE):
        with open(filename, "a", encoding="utf-8") as f:
            f.write("# Generated by LiveTesterV0.6. For authorized pentesting only.\n")
            json.dump(data, f)
            f.write("\n")
    ...
    def extract_url_parameters(self, url):
        ...
        # Extract fragment parameters, handling /path/page?param=value
        if parsed_url.fragment:
            fragment_query = parsed_url.fragment.split('?')[-1] if '?' in parsed_url.fragment else ''
    ...
    def sync_test_driver_session(self, domain=None):
        ...
        target_domain = domain or self.current_domain or "https://grupoigo.com.br/"
        if not self.is_valid_url(target_domain):
            logging.warning(f"Skipping session sync: invalid domain ({target_domain})")
            return
        try:
        ...
        try:
            ...
            if form and form.get("action"):
                ...
                script = """
                    ...
                    let inputData = arguments[0];
                    let actionUrl = arguments[1];
                    ...
                    inputData.forEach(data => {
                        ...
                        input.name = escapeHTML(data.name);
                        input.value = escapeHTML(data.value);
                ...
                """
                try:
                    self.test_driver.execute_script(script, input_data, action_url)
                except Exception as e:
                    logging.error(f"Error executing POST form script for {url} with param {param_name}: {e}")
                    return None
            else:
                script = """
                    function escapeHTML(str) {
                    ...
                    let paramName = arguments[0];
                    let payload = arguments[1];
                    let actionUrl = arguments[2];
                    let form = document.createElement('form');
                    form.id = 'xssForm';
                    form.method = 'POST';
                    form.action = actionUrl;
                    let input = document.createElement('input');
                    input.name = escapeHTML(paramName);
                    input.value = escapeHTML(payload);
                    form.appendChild(input);
                    document.body.appendChild(form);
                    document.getElementById('xssForm').submit();
                """
                try:
                    self.test_driver.execute_script(script, param_name, payload, url)
                except Exception as e:
                    logging.error(f"Error executing default POST form script for {url} with param {param_name}: {e}")
                    return None
            WebDriverWait(self.test_driver, 20).until(EC.presence_of_element_located((By.TAG_NAME, 'body')))
            if "captcha" in self.test_driver.page_source.lower():
