...
class LiveXSSTester:
    ...
    def append_to_file(self, data, filename=PARAMETERS_OUTPUT_FILE):
        with open(filename, "a", encoding="utf-8") as f:
            f.write("# Generated by LiveTesterV0.10. For authorized pentesting only.\n")
            json.dump(data, f)
            f.write("\n")
                ...
                for key in keys:
                    if '*' in key:
                        ...
                        if storage_items:
                            ...
                            break
            if tokens:
                self.test_driver.get(target_domain or self.current_domain or "about:blank")
                for key, value in tokens.items():
                    if value:
                        ...
                        """)
                logging.info(f"Synced tokens to test driver for {target_domain or self.current_domain}: {list(tokens.keys())}")
                ...
                logging.debug("No tokens found in localStorage/sessionStorage")
        except Exception as e:
            logging.warning(f"Failed to sync tokens from storage: {e}")
